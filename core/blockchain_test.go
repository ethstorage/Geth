...
func TestEIP6968(t *testing.T) {
	...
	state, _ := chain.State()
	want := uint64(83252400000000)
	if got := state.GetBalance(cc); got.Uint64() != want {
		t.Fatalf("failed to pay revenue recipient %s: got %d, want %d", cc, got, want)
	}
}
